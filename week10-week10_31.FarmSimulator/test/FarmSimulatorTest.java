...
public class FarmSimulatorTest<S, L, R, N, M> {
    ...
    private ClassRef<N> barnClassRef;
    private MethodRef0<N, S> barnGetBulkTankMethodRef;
    private MethodRef1<N, Void, R> barnAsennaMilkingRobotMethodRef;
    private MethodRef1<N, Void, L> barnHoidaCowMethodRef;
    private MethodRef1<N, Void, Collection> barnHoidaCowtMethodRef;
    private ClassRef<M> farmClassRef;
    private MethodRef0<M, String> farmGetOmistajaMethodRef;
    private MethodRef1<M, Void, R> farmAsennaBarnanMilkingRobotMethodRef;
    ...
    @Test
    @Points("31.4")
    public void testBarn() throws Throwable {
        reflektoiBulkTank();
        reflektoiCow();
        reflektoiMilkingRobot();
        reflektoiBarn();
        ...
        N barn = luoBarn(barnClassRef, tank);

        R robo = luoMilkingRobot(lypsyrobottiClassRef);

        try {
            barnAsennaMilkingRobotMethodRef.invokeOn(barn, robo);
        } catch (Throwable t) {
            Assert.fail("Class " + barn.getClass().getName() + "'s method "
                    + barnAsennaMilkingRobotMethodRef.getMethod().getName()
                    + " threw an exception: " + t.toString());
        }

        testaaNavetanLehmienHoitamista(barn, cows);
    }
    ...
    public void testFarm() throws Throwable {
        ...
        N barn = luoBarn(barnClassRef, tank);
        M farm = luoFarm(farmClassRef, "Öyvind", barn);

        String v = "\nBulkTank tank = new BulkTank();\n"
                + "Farm tila = new Farm(\"pekka\", "
                + "new farmsimulator.Barn( tank ));\n";
        ...
        farmAsennaBarnanMilkingRobotMethodRef.withNiceError(v).invokeOn(farm, robo);
    ...
    private void reflektoiBarn() {
        String luokanNimi = "farmsimulator.farmsimulator.Barn";

        barnClassRef = (ClassRef<N>) Reflex.reflect(luokanNimi);

        saniteettitarkastus(luokanNimi, 2, "instance variables for bulk tank and milking robot");

        barnGetBulkTankMethodRef = barnClassRef.method("getBulkTank").
                returning(bulkTankClassRef.getReferencedClass()).takingNoParams();
        Assert.assertTrue("Class " + s(luokanNimi) + " doesn't have method public " + barnGetBulkTankMethodRef.signature(),
                barnGetBulkTankMethodRef.isPublic());

        barnHoidaCowMethodRef = barnClassRef.method("takeCareOf").
                returningVoid().taking(cowClassRef.getReferencedClass());
        Assert.assertTrue("Class " + s(luokanNimi) + " doesn't have method public " + barnHoidaCowMethodRef.signature(),
                barnHoidaCowMethodRef.isPublic());

        barnHoidaCowtMethodRef = barnClassRef.method("takeCareOf").
        ...
        Assert.assertTrue("Class " + s(luokanNimi) + " doesn't have method public void takeCareOf(Collection<Cow> cows)",
                barnHoidaCowtMethodRef.isPublic());

        barnAsennaMilkingRobotMethodRef = barnClassRef.method("installMilkingRobot").
                returningVoid().taking(lypsyrobottiClassRef.getReferencedClass());
        Assert.assertTrue("Class " + s(luokanNimi) + " doesn't have method public " + barnAsennaMilkingRobotMethodRef.signature(),
                barnAsennaMilkingRobotMethodRef.isPublic());
    }

    private N luoBarn(ClassRef<N> luokka, S tank) throws Throwable {

        assertTrue("Create for class farmsimulator.Barn a constructor public farmsimulator.Barn(BulkTank tank)", luokka.constructor().taking(bulkTankClassRef.getReferencedClass()).isPublic());

        N instanssi = luokka.constructor().taking(bulkTankClassRef.getReferencedClass()).invoke(tank);

        String v = "BulkTank m = new BulkTank();\n"
                + "farmsimulator.Barn n = new farmsimulator.Barn(m);\n"
                + "n.getBulkTank();\n";

        S palautettuSailio = barnGetBulkTankMethodRef.withNiceError(v).invokeOn(instanssi);

        Assert.assertTrue("Class " + s(instanssi.getClass().getName())
                + "'s method " + barnGetBulkTankMethodRef.getMethod().getName()
        ...
        try {
            barnHoidaCowMethodRef.invokeOn(instanssi, cow);
            ...
            if (!t.toString().contains("IllegalState")) {
                Assert.fail("Class " + s(instanssi.getClass().getName())
                        ...
                        + "check code:\n"
                        + "farmsimulator.Barn barn = new farmsimulator.Barn( new BulkTank());\n"
    ...
    private void testaaNavetanLehmienHoitamista(N barn, List<L> cows) throws Throwable {
        ...
        String v = "\n"
                ...
                + "cow1.liveHour();"
                + "BulkTank m = new BulkTank();\n"
                + "farmsimulator.Barn nav = new farmsimulator.Barn(m);\n"
                + "nav.setMilkingRobot( new MilkingRobot() );\n"
                + "nav.takeCareOf(cow1);";

        barnHoidaCowMethodRef.withNiceError(v).invokeOn(barn, cows.get(0));

        S tank = barnGetBulkTankMethodRef.withNiceError(v + "\nnav.getBulkTank()").invokeOn(barn);

        double tanknSaldo = annaBulkTanknSaldo(tank);

        Assert.assertEquals("Executed code" + v + "\nSo one cow was taken care of in the barn which had "
        ...
        barnHoidaCowtMethodRef.withNiceError(v).invokeOn(barn, cows);

        tanknSaldo = annaBulkTanknSaldo(tank);

        Assert.assertEquals("Executed code\n"+v
                + "So multiple cows were taken care of in the barn and their total amount of milk was "
                + maaraLehmissaYhteensa + " litres but in the tank went "
                + "different amount of milk: " + tanknSaldo,
                maaraLehmissaYhteensa, tanknSaldo, 0.1);

        String tila = barn.toString();

        assertFalse("Create for class farmsimulator.Barn method toString() as defined in the assignment",
                tila.contains("@"));

        String[] tilanOsat = tila.split("/");

        Assert.assertTrue("Class " + barn.getClass().getName()
                + "'s method toSring() should return the state of the barn bulk tank "
                + "in the following way: volume/capacity, \n"
                + "but returned string was: " + tila,
                tilanOsat.length == 2);
        ...
        try {
            ...
            Assert.fail("Class " + barn.getClass().getName()
        ...
        testaaOnkoKokonaisLuku(barn.getClass(), "toString()",
                merkkijonoSaldo, tila);
        testaaOnkoKokonaisLuku(barn.getClass(), "toString()",
                merkkijonoTilavuus, tila);

        Assert.assertEquals("Class " + barn.getClass().getName()
                + "'s method toSring() should return the state of the barn bulk tank "
                + "in the following way: volume/capacity, but returned volume was wrong: "
    ...
    private void reflektoiFarm() {
        ...
        saniteettitarkastus(luokanNimi, 3, "instance variables for owner, barn and cows. ");
        ...
        farmAsennaBarnanMilkingRobotMethodRef = farmClassRef.method("installMilkingRobot").
                returningVoid().taking(lypsyrobottiClassRef.getReferencedClass());
        Assert.assertTrue("Class " + s(luokanNimi) + " doesn't have method public " + farmAsennaBarnanMilkingRobotMethodRef.signature(),
                farmAsennaBarnanMilkingRobotMethodRef.isPublic());
    }

    private M luoFarm(ClassRef<M> luokka, String owner, N barn) throws Throwable {

        String v = "Farm tila = new Farm(\"" + owner + "\", new farmsimulator.Barn( new BulkTank() ));\n";

        assertTrue("Create for class " + s(luokka.getReferencedClass().getName())
                + " a constructor public Farm(String owner, farmsimulator.Barn barn)", luokka.constructor().taking(String.class,
                barnClassRef.getReferencedClass()).isPublic());

        M instanssi = luokka.constructor().taking(String.class,
                barnClassRef.getReferencedClass()).withNiceError(v).invoke(owner, barn);
    ...
    private void testaaFarmnLehmienHoitamista(M farm, S tank,
        ...
        String v = "BulkTank tank = new BulkTank();\n"
                + "Farm tila = new Farm(\"pekka\", new farmsimulator.Barn( tank ));\n"
